...
class AiView(ViewSet):
    ...
    def create(self, request):
        ...
        instructions = [
            ...
            {"role": "user", "content": f"You are playing chess against the user as {color} and trying to win the game. The current PGN is {pgn}. The current FEN notation is {fen}. You MUST pick from one of the available moves listed in {possible_moves}. Take some time to think about the best move and then pick a move from the list that maximizes your chance of winning. Return your response in the same format as this example: 'move: e5'. Pick a move even if you don't have a good option and do not provide any data outside of the provided format."}
        ]
        # try:
